...
const getEndpointSnippets = function (openApi, path, method, targets, values) {
  ...
  // skips any post endpoints that don't have json content type
  if (!OpenAPIToHar.isEndpointValid(openApi, path, method)) {
    return {
...
const getSnippets = function (openApi, targets) {
  ...
  for (let i in endpointHarInfoList) {
    ...
    // if harinfo method is null, dont add to results
    if (!harInfo.method) {
      continue;
    }
