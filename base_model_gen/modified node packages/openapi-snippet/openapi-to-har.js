...
/**
 * Checks if the endpoint is valid based on the method type and content type.
 * 
 * @param {Object} openApi - OpenAPI document
 * @param {string} path - Path of the endpoint
 * @param {string} method - HTTP method of the endpoint
 * @returns {boolean} - Returns true if the endpoint meets the criteria, false otherwise
 */
const isEndpointValid = function (openApi, path, method) {
...
/**
 * Generates dummy data based on the property description.
 * 
 * @param {Object} property - The property object from the OpenAPI spec
 * @returns {*} - The generated dummy data
 */
const generateDummyData = function (property, type) {
  switch (type) {
    case 'integer':
      return Math.floor(Math.random() * 100); // Random integer between 0 and 99
    case 'number':
      return Math.random() * 100; // Random number between 0 and 99.999...
    case 'string':
      if (property.format === 'date-time') {
        return new Date().toISOString(); // Current date-time in ISO format
      } else if (property.format === 'email') {
        return 'example@example.com'; // Example email
      } else if (property.format === 'byte') {
        return generateRandomByteString(20); // Random byte string
      } else {
        return generateRandomString(20); // Random alphabetic string
      }
    case 'boolean':
      return Math.random() < 0.5; // Random boolean
    case 'array':
      if (property.items) {
        return [generateDummyData(property.items)]; // Generate an array with one item
      }
      return [];
    case 'object':
      if (property.properties) {
        ...
        for (const [key, value] of Object.entries(property.properties)) {
          obj[key] = generateDummyData(value);
        }
        return obj;
      }
      return {};
...
/**
 * Generates sample data for a given pattern.
 * @param {string} pattern - The regex pattern to generate data for.
 * @returns {string} - The generated sample data.
 */
const generateSampleData = function (pattern) {
  // Example implementation for pattern: ^[\\da-z]{26}$
  if (pattern === '^[\\da-z]{26}$') {
...
const getParameterValues = function (openApi, param, location, values) {
  ...
  //   value = `{${param.name}}`;
  // }
  let type = param.type || param.schema.type;
  ...
  if (values && typeof values[param.name] !== 'undefined') {
    ...
    value = param.default;
  } else if (typeof param.schema !== 'undefined' && typeof param.schema.pattern !== 'undefined') {
    value = generateSampleData(param.schema.pattern);
  } else {
    value = generateDummyData(param.schema || param, type);
  }
